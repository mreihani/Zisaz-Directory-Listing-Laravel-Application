    ...
    <div class="container pt-4 pb-5 mb-md-4">
        ...
        <nav class="pb-4 my-2" aria-label="Breadcrumb">
            <ol class="breadcrumb m-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">کارجو</li>
            </ol>
        </nav>
        <div class="row">
            <!-- Signle job content-->
            <div class="col-lg-7 position-relative pe-lg-5 mb-5 mb-lg-0" style="z-index: 1025;">
                ...
                @if($activity->activityGroups->count())
                    ...
                    <ul class="list-unstyled">
                        @foreach ($activity->activityGroups as $activityGroupItem)
                            <li class="d-flex">
                ...
                @if($activity->contractType->count())
                    <h3 class="h6 pt-2">نوع همکاری: </h3>
                    <ul class="list-unstyled">
                        @foreach ($activity->contractType as $contractTypeItem)
                            <li class="d-flex">
                                <span class="text-primary fs-lg me-2">•</span>
                ...
                @if($activity->academic->count())
                    <h3 class="h6">تحصیلات: </h3>
                    <p class="line-h18">
                        {{$activity->academic->first()->title}}
                    </p>
                @endif

                @if($activity->subactivity->age)
                    <h3 class="h6">سن: </h3>
                    <p class="line-h18">
                        {{$activity->subactivity->age}}
                    </p>
                @endif

                @if($activity->gender->count())
                    <h3 class="h6">جنسیت: </h3>
                    <p class="line-h18">
                        {{$activity->gender->first()->title}}
                    </p>
                @endif

                @if($activity->subactivity->work_exp)
                    <h3 class="h6">سابقه کار: </h3>
                    <p class="line-h18">
                ...
                @if($activity->province->count())
                    <h3 class="h6">استان هایی که می توانم مشغول به کار شوم: </h3>
                    <ul class="list-unstyled">
                        @foreach ($activity->province as $provinceItem)
                            <li class="d-flex">
                                <span class="text-primary fs-lg me-2">•</span>
                                {{$provinceItem->title}}
                            </li> 
    ...
    <section class="container pt-md-2 pb-5 mb-md-4">
        <div class="d-sm-flex align-items-center justify-content-between pb-4 mb-sm-2">
            ...
            <a class="btn btn-link fw-normal p-0" 
                href="{{route('get-activities', ['activity_type' => 'ads_registration', 'r_name' => 'employment', 'ads_type' => 'employee'])}}">
                مشاهده همه
