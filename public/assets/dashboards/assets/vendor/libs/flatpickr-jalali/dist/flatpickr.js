...
var revFormat = {
    ...
    W: function (dateObj, weekNum) {
        ...
        return new JDate(dateObj.getFullYear(), 0, 2 + (weekNumber - 1) * 7, 0, 0, 0, 0);
    },
...
function FlatpickrInstance(element, instanceConfig) {
        ...
        self.utils = {
            getDaysInMonth: function (month, yr) {
                ...
                if (self.config.locale === "fa") {
                    var calcedYear = yr - Math.floor(month / 12);
    ...
    function updateTime(e) {
        if (self.config.noCalendar && self.selectedDates.length === 0) {
            self.setDate(self.config.minDate !== undefined
                ...
                : new JDate().setHours(self.config.defaultHour, self.config.defaultMinute, self.config.defaultSeconds, 0), false);
            setHoursFromInputs();
            updateValue();
        }
        timeWrapper(e);
    ...
    function buildDays() {
        ...
        var firstOfMonth = (new JDate(self.currentYear, self.currentMonth, 1).getDay() -
        ...
        if (isRangeMode) {
            self.minRangeDate = new JDate(self.currentYear, self.currentMonth - 1, dayNumber);
            self.maxRangeDate = new JDate(self.currentYear, self.currentMonth + 1, (42 - firstOfMonth) % daysInMonth);
        }
        for (; dayNumber <= prevMonthDays; dayNumber++, dayIndex++) {
            days.appendChild(createDay("prevMonthDay", new JDate(self.currentYear, self.currentMonth - 1, dayNumber), dayNumber, dayIndex));
        }
        for (dayNumber = 1; dayNumber <= daysInMonth; dayNumber++, dayIndex++) {
            days.appendChild(createDay("", new JDate(self.currentYear, self.currentMonth, dayNumber), dayNumber, dayIndex));
        }
        for (var dayNum = daysInMonth + 1; dayNum <= 42 - firstOfMonth; dayNum++, dayIndex++) {
            days.appendChild(createDay("nextMonthDay", new JDate(self.currentYear, self.currentMonth + 1, dayNum % daysInMonth), dayNum, dayIndex));
        }
        if (isRangeMode && self.selectedDates.length === 1 && days.childNodes[0]) {
            ...
            self._hideNextMonthArrow =
                self._hideNextMonthArrow ||
                    (!!self.maxRangeDate &&
                        self.maxRangeDate <
                            new JDate(self.currentYear, self.currentMonth + 1, 1));
        }
        else
