...
(async function () {
  ...
  function renderBoardDropdown() {
    return (
      ...
      "<a class='dropdown-item delete-board' href='javascript:void(0)'> <i class='ti ti-trash ti-xs me-1' ></i> <span class='align-middle'>حذف</span></a>" +
      "<a class='dropdown-item' href='javascript:void(0)'><i class='ti ti-edit ti-xs me-1' ></i> <span class='align-middle'>تغییر نام</span></a>" +
      "<a class='dropdown-item' href='javascript:void(0)'><i class='ti ti-archive ti-xs me-1' ></i> <span class='align-middle'>بایگانی</span></a>" +
      '</div>' +
      '</div>'
    );
  }
  ...
  function renderDropdown() {
    return (
      ...
      "<a class='dropdown-item' href='javascript:void(0)'>کپی لینک تسک</a>" +
      "<a class='dropdown-item' href='javascript:void(0)'>کپی کردن تسک</a>" +
  ...
  const kanban = new jKanban({
    ...
    click: function (el) {
      ...
      let title = element.getAttribute('data-eid')
        ...
        dateToUse = date
          ? date + ', ' + year
          : dateObj.getDate() + ' ' + dateObj.toLocaleString('fa', { month: 'long' }) + ', ' + year,
    ...
    buttonClick: function (el, boardId) {
      ...
      addNew.innerHTML =
        ...
        '</div>' +
        '<div class="mb-3">' +
        '<button type="submit" class="btn btn-primary btn-sm me-2 waves-effect waves-light">افزودن</button>' +
        '<button type="button" class="btn btn-label-secondary btn-sm cancel-add-item waves-effect waves-light">لغو</button>' +
        '</div>';
